...
void ESPDashClass::onWsEvent(AsyncWebSocket *server,
...
#if defined(DEBUG_MODE)
            Serial.println("[WEBSOCKET] Got getLayout Command from Client " +
                           String(client->id()));
#endif
            String result = "";
...
#if defined(DEBUG_MODE)
            Serial.println("[WEBSOCKET] Got getStats Command from Client " +
                           String(client->id()));
#endif
            String result = "";
...
#if defined(DEBUG_MODE)
              Serial.println("buttonClicked Command didn't match any ID in our "
                             "records! Rouge Request...");
#endif
            }
            ...
            if (ESPDash._sliderChangedFunc != NULL) {
              ...
              for (int i = 0; i < SLIDER_CARD_LIMIT; i++) {
                if (ESPDash.slider_card_id[i] == sliderId) {
                  ...
                  ws.textAll("{\"response\": \"updateSliderCard\", \"id\": \"" +
                             (String)sliderId +
                             "\", \"value\": " + sliderValue + "}");
                  return;
                }
...
#if defined(DEBUG_MODE)
              Serial.println("sliderChanged Command didn't match any ID in our "
                             "records! Rouge Request...");
#endif
            }
          }
...
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Number Cards. Inserted "
                       "New Card at Index [" +
                       String(i) + "].");
#endif
...
void ESPDashClass::updateIRArrayCard(const char *_id, int _value[], size_t sizeArray) {
  for (int i = 0; i < IRARRAY_CARD_LIMIT; i++) {
    if (irarray_card_id[i] == _id) {
      ...
      values += String(_value[sizeArray - 1]);

      Serial.println(values);
      irarray_card_value[i] = values;
...
void ESPDashClass::addTemperatureCard(const char *_id, const char *_name,
                                      int _type) {
  if (_id != NULL && _type >= 0 && _type <= TEMPERATURE_CARD_TYPES) {
    for (int i = 0; i < TEMPERATURE_CARD_LIMIT; i++) {
      if (temperature_card_id[i] == "") {
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Temperature Cards. "
                       "Inserted New Card at Index [" +
                       String(i) + "].");
#endif
...
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Humidity Cards. Inserted "
                       "New Card at Index [" +
                       String(i) + "].");
#endif
...
void ESPDashClass::addStatusCard(const char *_id, const char *_name) {
  if (_id != NULL) {
    for (int i = 0; i < STATUS_CARD_LIMIT; i++) {
      if (status_card_id[i] == "") {
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Status Cards. Inserted "
                       "New Card at Index [" +
                       String(i) + "].");
#endif
...
void ESPDashClass::addButtonCard(const char *_id, const char *_name) {
  if (_id != NULL) {
    for (int i = 0; i < BUTTON_CARD_LIMIT; i++) {
      if (button_card_id[i] == "") {
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Button Cards. Inserted "
                       "New Card at Index [" +
                       String(i) + "].");
#endif
...
// Add Slider Card
void ESPDashClass::addSliderCard(const char *_id, const char *_name,
                                 int _type) {
  if (_id != NULL && _type >= 0 && _type <= SLIDER_CARD_TYPES) {
    for (int i = 0; i < SLIDER_CARD_LIMIT; i++) {
      if (slider_card_id[i] == "") {
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Slider Cards. Inserted "
                       "New Card at Index [" +
                       String(i) + "].");
#endif
...
// Add Line Chart
void ESPDashClass::addLineChart(const char *_id, const char *_name,
                                int _x_axis_value[], int _x_axis_size,
                                const char *_y_axis_name, int _y_axis_value[],
                                int _y_axis_size) {
  if (_id != NULL) {
    for (int i = 0; i < LINE_CHART_LIMIT; i++) {
      if (line_chart_id[i] == "") {
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Line Charts. Inserted "
                       "New Chart at Index [" +
                       String(i) + "].");
#endif
...
// Add Line Chart
void ESPDashClass::addLineChart(const char *_id, const char *_name,
                                String _x_axis_value[], int _x_axis_size,
                                const char *_y_axis_name, int _y_axis_value[],
                                int _y_axis_size) {
  if (_id != NULL) {
    for (int i = 0; i < LINE_CHART_LIMIT; i++) {
...
void ESPDashClass::updateLineChart(const char *_id, int _x_axis_value[],
...
void ESPDashClass::updateLineChart(const char *_id, String _x_axis_value[],
                                   int _x_axis_size, int _y_axis_value[],
                                   int _y_axis_size) {
  for (int i = 0; i < LINE_CHART_LIMIT; i++) {
    if (line_chart_id[i] == _id) {
...
void ESPDashClass::addGaugeChart(const char *_id, const char *_name) {
  if (_id != NULL) {
    for (int i = 0; i < GAUGE_CHART_LIMIT; i++) {
      if (gauge_chart_id[i] == "") {
#if defined(DEBUG_MODE)
        Serial.println("[DASH] Found an empty slot in Gauge Cards. Inserted "
                       "New Card at Index [" +
                       String(i) + "].");
#endif
...
#if defined(DEBUG_MODE)
  Serial.println("Free HEAP = before = JSON Serialization: " +
...
#if defined(DEBUG_MODE)
  Serial.println("Free HEAP = after = JSON Serialization: " +
                 String(ESP.getFreeHeap()));
#endif
...
size_t ESPDashClass::getTotalResponseCapacity() {
  ...
  for (int i = 0; i < LINE_CHART_LIMIT; i++) {
    if (line_chart_id[i] != "") {
      capacity += JSON_OBJECT_SIZE(4) +
                  JSON_ARRAY_SIZE(line_chart_x_axis_size[i]) +
                  JSON_ARRAY_SIZE(line_chart_y_axis_size[i]);
      totalCards++;
    }
  }
